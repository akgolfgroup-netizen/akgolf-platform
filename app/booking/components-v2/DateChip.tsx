...
export function DateChip({ dayName, dayNumber, month, isSelected, onClick }: DateChipProps) {
  return (
    ...
    >
      <div className={`text-[10px] font-semibold uppercase ${isSelected ? "text-surface/60" : "text-on-surface-variant"}`}>
        {dayName}
      </div>
      <div className={`text-lg font-bold mt-0.5 ${isSelected ? "text-surface" : "text-on-surface"}`}>
        {dayNumber}
      </div>
      <div className={`text-[10px] ${isSelected ? "text-secondary-fixed" : "text-on-surface-variant"}`}>
        {month}
      </div>
    </button>
  );
