      ...
      .then((data) => {
        ...
        if (foundService) {
          ...
          if (instructorIdParam) {
            targetInstructor = data.flatMap((s: ServiceType & { Instructor?: Instructor[] }) => s.Instructor ?? []).find((i: Instructor) => i.id === instructorIdParam);
          }
      ...
      <main className="lg:ml-64 min-h-screen bg-surface">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-12">
          ...
          <div className="lg:hidden mb-6">
            <span className="font-mono text-xs text-primary uppercase tracking-widest">Steg 2 av 3</span>
            <h1 className="text-2xl font-bold text-primary tracking-tight mt-1">Velg tid</h1>
          </div>

          {/* Desktop Title */}
          <div className="hidden lg:block mb-6">
            <span className="font-mono text-xs text-primary uppercase tracking-widest">Steg 2 av 3</span>
            <h1 className="text-4xl font-bold text-primary tracking-tight mt-1">Velg tid</h1>
          </div>

          {/* Coach Info Card */}
          <div className="bg-white rounded-2xl p-4 lg:p-6 border border-primary/10 shadow-sm mb-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 lg:w-14 lg:h-14 rounded-xl bg-primary flex items-center justify-center flex-shrink-0">
                <User className="w-6 h-6 lg:w-7 lg:h-7 text-accent-cta" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-mono uppercase text-muted tracking-wider">Din coach</p>
                <p className="text-base lg:text-lg font-bold text-primary truncate">{instructor?.User?.name || "Instruktør"}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-[10px] font-mono uppercase text-muted tracking-wider">Varighet</p>
                <p className="text-base lg:text-lg font-bold text-primary">{service?.duration || 20} min</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <button
              ...
              className="flex items-center gap-1 lg:gap-2 px-3 lg:px-4 py-2 rounded-lg font-medium text-sm text-primary hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              <ChevronLeft className="w-5 h-5" />
              <span className="hidden sm:inline">Forrige uke</span>
            </button>
            <div className="text-center">
              <h2 className="text-lg lg:text-xl font-bold text-primary">{getWeekLabel()}</h2>
              <p className="text-xs text-muted font-mono uppercase tracking-wider">Uke {weekOffset + 1}</p>
            </div>
            <button
              onClick={goToNextWeek}
              className="flex items-center gap-1 lg:gap-2 px-3 lg:px-4 py-2 rounded-lg font-medium text-sm text-primary hover:bg-white transition-colors"
            >
          ...
          {loadingSlots ? (
            ...
            <div className="bg-white rounded-2xl p-4 lg:p-6 border border-primary/10 shadow-sm">
              {/* Weekdays */}
                    ...
                    <div
                      ...
                      className={`rounded-xl p-3 lg:p-4 transition-all ${
                        isSelected
                          ...
                          : hasAvailableSlots
                            ? 'bg-surface hover:bg-accent-cta/20 cursor-pointer'
                            : 'bg-surface/50 opacity-60'
                      }`}
                    >
                      <div className="text-center mb-2 lg:mb-3">
                        <p className={`text-[10px] font-mono uppercase tracking-wider ${isSelected ? 'text-white/70' : 'text-muted'}`}>
                          {day.dayName}
                        </p>
                        <p className={`text-xl lg:text-2xl font-bold ${isSelected ? 'text-white' : 'text-primary'}`}>
                          {day.dayNumber}
                        </p>
                      </div>
                          ...
                          return (
                            <button
                              ...
                              className={`w-full py-1.5 lg:py-2 rounded-lg font-mono text-xs lg:text-sm transition-all ${
                                isTimeSelected
                                  ? 'bg-accent-cta text-primary font-bold'
                                  : slot.available
                                    ? isSelected
                                      ? 'bg-white/20 text-white hover:bg-white/30'
                                      : 'bg-white border border-primary/30 text-primary hover:border-primary'
                                    : 'bg-transparent text-muted line-through cursor-not-allowed'
                              }`}
                            >
                              {slot.time}
                            </button>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2 lg:mb-3">
                        ...
                        <Clock className={`w-5 h-5 ${isSelected ? 'text-white/50' : 'text-muted'}`} />
                      </div>
          ...
          {selectedSlot?.available && selectedDate && (
            <div className="mt-6 lg:mt-8 bg-primary p-4 lg:p-6 rounded-2xl text-white shadow-xl">
              <div className="flex justify-between items-start mb-4 lg:mb-6">
                ...
                <BadgeCheck className="w-6 h-6 text-accent-cta flex-shrink-0" />
              </div>
              <button
                onClick={handleContinue}
                className="w-full bg-accent-cta text-primary font-bold uppercase tracking-widest py-3 lg:py-4 rounded-xl flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all"
              >
          ...
          <div className="mt-6 lg:mt-8 bg-surface rounded-xl p-4 lg:p-6">
            <div className="flex items-start gap-3">
              <div className="w-6 h-6 rounded-full bg-accent-cta flex items-center justify-center flex-shrink-0 mt-0.5">
                <Check className="w-4 h-4 text-primary" />
              </div>
              <div>
                <p className="font-bold text-primary text-sm">Forenklet booking</p>
                <p className="text-text text-sm mt-1">
                  Vi viser kun de mest populære tidspunktene (10:00, 13:00, 15:00, 17:00).
                  Trenger du et annet tidspunkt? Kontakt oss på <a href="mailto:anders@akgolf.no" className="text-primary underline">anders@akgolf.no</a>
                </p>
              </div>
            </div>
          </div>
