...
function formatBookingWhen(when: string | Date): string {
  ...
  const day = d.toLocaleDateString("nb-NO", { weekday: "long", day: "2-digit", month: "short" });
  return `${day} · ${time}`;
}

export function DashboardV2Client(props: DashboardV3Props) {
  const {
    userName,
    stats,
    handicap,
  ...
  const subline = coachInsight?.primaryFocus
    ? `Fokus denne uken: ${coachInsight.primaryFocus}.`
    : trainingIndex
      ? `Planoppfyllelse: ${trainingIndex.planAdherencePct}%. Hold tempoet.`
      : null;

  const handicapValue = handicap.current !== null ? handicap.current.toFixed(1) : "—";
  const handicapTrend = handicap.trend;
  const handicapTrendText =
    handicapTrend !== null ? `${Math.abs(handicapTrend).toFixed(1)}` : undefined;

  const planPct = trainingIndex?.planAdherencePct ?? null;

  const heroStats = [
    { label: "Handicap", value: handicapValue, unit: handicapTrendText ? `${handicapTrend! < 0 ? "↓" : "↑"} ${handicapTrendText}` : undefined },
    { label: "SG Total", value: sgDelta !== null ? (sgDelta >= 0 ? `+${sgDelta.toFixed(1)}` : sgDelta.toFixed(1)) : "—", unit: `siste ${sgSummary.roundCount || 0}r` },
    { label: "Streak", value: String(socialData?.streak ?? 0), unit: "dager" },
    { label: "Plan", value: planPct !== null ? `${planPct}%` : "—", unit: planPct !== null ? "fullført" : undefined },
  ];
        ...
        <KpiCard
          ...
          change={
            handicapTrend !== null && handicapTrendText
              ? { text: handicapTrendText, direction: handicapTrend < 0 ? "down" : handicapTrend > 0 ? "up" : "flat", isGood: handicapTrend < 0 }
              : undefined
          }
          contextLabel="siste 30d"
          sparkline={handicapHistory.length > 0 ? { points: handicapHistory, type: "line" } : undefined}
        />

        <KpiCard label="Runder · 30d" value={String(stats.roundsCount)} contextLabel="totalt registrert" />
        <KpiCard label="Treningsøkter" value={String(stats.sessionsCount)} contextLabel="totalt" />

        <KpiCard
          label="SG Approach"
          value={sgSummary.approach !== null ? (sgSummary.approach >= 0 ? `+${sgSummary.approach.toFixed(2)}` : sgSummary.approach.toFixed(2)) : "—"}
