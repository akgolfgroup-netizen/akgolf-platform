...
const PYRAMID_COLORS: Record<string, string> = {
  FYS: "var(--color-error)",      // #B84233
  TEK: "var(--color-warning)",    // #C48A32
  SLAG: "var(--color-primary)",   // #005840
  SPILL: "var(--color-info)",     // #007AFF
  TURN: "var(--color-ai)",        // #AF52DE
};
...
const focColor = (f: string): string => PYRAMID_HEX[f] || "#005840";
...
function HelpBtn({ title, items, color }: { title: string; items: RefItem[]; color?: string }) {
  ...
  const btnColor = color || "var(--color-portal-muted)";
  return (
    <span className="relative inline-flex">
      ...
      {open && (
        <>
          ...
          <div onClick={(e) => e.stopPropagation()} className="absolute top-6 -left-2.5 w-[280px] bg-white rounded-[14px] shadow-[0_8px_32px_rgba(0,0,0,.18)] z-[999] p-3.5 border border-portal-border">
            <div className="flex justify-between mb-2">
              <div className="text-[13px] font-bold">{title}</div>
              <button onClick={() => setOpen(false)} className="bg-transparent border-none cursor-pointer text-base text-portal-muted">x</button>
            </div>
            <div className="max-h-[220px] overflow-y-auto">
              {items.map((it, i) => (
                <div key={i} className={`py-1.5 ${i > 0 ? "border-t border-black/4" : ""}`}>
                  ...
                  <div className="text-[10px] text-portal-secondary mt-0.5 leading-[1.4]">{it.desc}</div>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className={compact ? "mb-2" : "mb-3"}>
      <div className="flex items-center gap-1.5 mb-1.5">
        <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted">{label}</div>
          ...
          return (
            <button key={code} onClick={() => onChange(code)}
              ...
              style={{
                ...
                background: sel ? `${color}15` : "var(--color-portal-hover)",
                color: sel ? color : "var(--color-portal-secondary)",
...
function SlagFocusPick({ selected, onChange, color, compact }: {
  ...
  return (
    <div className={compact ? "mb-2" : "mb-3"}>
      <div className="flex items-center gap-1.5 mb-1.5">
        <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted">Slagfokus</div>
          ...
          return (
            <button key={sf.code} onClick={() => toggle(sf.code)}
              ...
              style={{
                ...
                background: sel ? `${color}12` : "var(--color-portal-bg)",
                border: sel ? `2px solid ${color}` : "2px solid var(--color-portal-hover)",
              }}
            >
              <div className="flex items-center gap-1.5">
                <span className="text-sm">{sf.icon}</span>
                <div className="text-[11px] font-bold" style={{ color: sel ? color : "var(--color-portal-text)" }}>{sf.name}</div>
                {sel && (
...
function PPosPick({ from, to, onChange, color }: {
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center gap-1.5 mb-1.5">
        <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted">P-Posisjon</div>
      ...
      {([["Fra", from, (v: string) => onChange(v, to)] as const, ["Til", to, (v: string) => onChange(from, v)] as const]).map(([l, val, set]) => (
        <div key={l} className="flex items-center gap-1.5 mb-1">
          <div className="text-[10px] text-portal-secondary font-semibold w-6">{l}</div>
          <div className="flex flex-wrap gap-1">
            {P_POS.map((p) => {
              ...
              return (
                <button key={p.code} onClick={() => set(p.code)}
                  ...
                  style={{
                    ...
                    background: sel ? `${color}15` : "var(--color-portal-hover)",
                    color: sel ? color : "var(--color-portal-muted)",
...
function ExerciseParams({ ex, onChange, compact }: {
  ...
  return (
    <div className="p-3 bg-portal-bg rounded-[10px]">
      {isTek && (
        <>
...
function ExerciseAddFlow({ onAdd, onClose }: { onAdd: (ex: V2Exercise) => void; onClose: () => void }) {
  ...
  const color = pyr ? focColor(pyr) : "var(--color-primary)";

  return (
    <div>
      {step === 0 && (
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">1. Pyramideniva</div>
          <div className="grid grid-cols-5 gap-1.5">
            {PYRAMID.map((p) => (
              ...
              >
                ...
                <div className="text-[9px] text-portal-muted mt-0.5">{p.full}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 1 && pyr && (
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            <button onClick={() => setStep(0)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-portal-secondary border-none cursor-pointer">&#8592;</button>
            <span className="text-xs font-bold" style={{ color }}>{pyr}</span>
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">2. Treningsomrade</div>
          <div className="grid grid-cols-3 gap-1.5 max-h-[200px] overflow-y-auto">
            {(AREAS[pyr] || []).map((a) => (
              <button key={a.code} onClick={() => pickArea(a.code)}
                className="py-2 px-1.5 rounded-lg border border-portal-border bg-white cursor-pointer text-center text-[11px] font-semibold hover:bg-portal-hover transition-colors"
              >{a.name}</button>
            ))}
          </div>
        </div>
      ...
      {step === 2 && pyr && area && (
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            <button onClick={() => setStep(1)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-portal-secondary border-none cursor-pointer">&#8592;</button>
            <span className="text-xs font-bold" style={{ color }}>{pyr} · {area}</span>
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">3. Konfigurer</div>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ovelsesnavn"
            className="w-full py-2.5 px-3.5 rounded-[10px] border border-portal-border text-sm outline-none mb-3 box-border bg-white"
          />
          <ExerciseParams
          ...
          <button onClick={confirm}
            className="w-full mt-2 py-3 rounded-[20px] bg-accent-cta text-accent-cta-text text-sm font-bold border-none cursor-pointer hover:opacity-85 transition-opacity"
          >Legg til</button>
        </div>
      )}
    </div>
...
function ExerciseCard({ ex, onRemove, onUpdate, showReps, editId, setEditId }: {
  ...
  return (
    ...
    >
      <button onClick={() => onRemove(ex.id)} className="absolute top-2 right-2.5 bg-transparent border-none cursor-pointer text-base text-portal-muted hover:text-error transition-colors">x</button>
      <div className="flex items-center gap-1.5 mb-0.5 pr-5">
        <div className="text-[13px] font-bold flex-1 text-portal-text">{ex.name}</div>
        <button onClick={() => setEditId(isEdit ? null : ex.id)}
          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-[20px] text-[9px] font-semibold cursor-pointer border-none transition-colors"
          style={{ background: isEdit ? `${ec}15` : "var(--color-portal-hover)", color: isEdit ? ec : "var(--color-portal-muted)" }}
          ...
          <div className="flex-1 bg-success/5 rounded-xl py-2 text-center border border-success/20">
            ...
            <div className="flex items-center justify-center gap-2.5">
              <button onClick={() => onUpdate(ex.id, { baller: Math.max(0, ex.baller - 1) })}
                className="w-[30px] h-[30px] rounded-full border border-portal-border bg-white cursor-pointer text-base font-bold text-portal-secondary flex items-center justify-center"
              >-</button>
              ...
              <button onClick={() => onUpdate(ex.id, { baller: ex.baller + 1 })}
                className="w-11 h-11 rounded-full border-none bg-success cursor-pointer text-xl font-bold text-white flex items-center justify-center shadow-[0_2px_8px_rgba(42,125,90,.25)]"
              >+</button>
            </div>
          </div>
          <div className="flex-1 bg-primary/5 rounded-xl py-2 text-center border border-primary/15">
            <div className="text-[9px] font-bold text-primary uppercase tracking-[0.08em] mb-1">Bevegelser</div>
            <div className="flex items-center justify-center gap-2.5">
              ...
              <button onClick={() => onUpdate(ex.id, { bevegelser: ex.bevegelser + 1 })}
                className="w-11 h-11 rounded-full border-none bg-primary cursor-pointer text-xl font-bold text-white flex items-center justify-center shadow-[0_2px_8px_rgba(0,88,64,.18)]"
              >+</button>
            </div>
          </div>
        </div>
    ...
    <div className="max-w-[600px] mx-auto p-6">
      <div className="bg-white rounded-xl shadow-card p-5">
        <div className="text-center mb-5">
          ...
          <div className="text-[44px] font-extrabold tabular-nums mt-2 text-primary">{fmtEl(elapsed)}</div>
        </div>
        <div className="flex justify-center gap-8 mb-6">
          <div className="text-center"><div className="text-[32px] font-extrabold">{totalB}</div><div className="text-[11px] text-portal-muted">Baller</div></div>
          <div className="text-center"><div className="text-[32px] font-extrabold">{totalM}</div><div className="text-[11px] text-portal-muted">Bevegelser</div></div>
        </div>
        ...
        <div className="flex justify-center mt-6">
          <button onClick={handleSaveAndClose}
            className="px-8 py-3 rounded-[20px] bg-accent-cta text-accent-cta-text text-[15px] font-bold border-none cursor-pointer hover:opacity-85 transition-opacity"
    ...
    <div className="max-w-[600px] mx-auto px-4 pt-4 pb-[120px]">
      <div className="bg-white rounded-xl shadow-card text-center mb-4 relative overflow-hidden p-5">
        ...
        <div className="text-[52px] font-extrabold tabular-nums tracking-tight leading-none mt-2"
          style={{ color: session.paused ? "var(--color-portal-muted)" : "var(--color-portal-text)" }}
        >{fmtEl(elapsed)}</div>
        <div className="text-[11px] text-portal-muted mt-1">{session.paused ? "PAUSET" : "AKTIV OKT"}</div>
        <div className="flex justify-center gap-7 mt-3">
          <div><div className="text-[22px] font-extrabold tabular-nums text-success">{totalB}</div><div className="text-[9px] text-portal-muted uppercase">Baller</div></div>
          <div><div className="text-[22px] font-extrabold tabular-nums">{totalM}</div><div className="text-[9px] text-portal-muted uppercase">Beveg.</div></div>
          <div><div className="text-[22px] font-extrabold tabular-nums">{session.exercises.length}</div><div className="text-[9px] text-portal-muted uppercase">Ovelser</div></div>
        </div>
        <div className="flex justify-center gap-2 mt-3 pb-1">
          <button onClick={togglePause}
            className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold border-none cursor-pointer"
            style={{ background: session.paused ? "var(--color-accent-cta)" : "var(--color-portal-hover)", color: session.paused ? "var(--color-primary)" : "var(--color-portal-secondary)" }}
          >{session.paused ? "Fortsett" : "Pause"}</button>
          <button onClick={() => setShowSummary(true)}
            className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-error border-none cursor-pointer"
          >Avslutt</button>
        </div>
      </div>
      {session.exercises.map((ex) => (
      ...
      {!showAdd ? (
        <button onClick={() => setShowAdd(true)}
          className="w-full mt-2 py-3.5 rounded-[14px] border-2 border-dashed border-portal-border bg-transparent cursor-pointer text-sm font-semibold text-portal-secondary hover:bg-portal-hover transition-colors"
        ...
        <div className="bg-white rounded-xl shadow-card mt-2 p-4">
          <div className="flex justify-between mb-3">
            ...
            <button onClick={() => setShowAdd(false)} className="bg-transparent border-none cursor-pointer text-lg text-portal-muted">x</button>
          </div>
    ...
    <div className="max-w-[600px] mx-auto px-4 pt-4 pb-[100px]">
      <div className="bg-white rounded-xl shadow-card p-5 mb-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onCancel} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-portal-secondary border-none cursor-pointer">Avbryt</button>
        </div>
        <div className="mb-3.5">
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">Oktnavn</div>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="F.eks. TEK Innspill 150m"
            className="w-full py-2.5 px-3.5 rounded-[10px] border border-portal-border text-sm outline-none box-border bg-white"
          />
        </div>
        <div className="flex gap-3 mb-3.5">
          <div className="flex-1">
            <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">Starttid</div>
            <div className="flex gap-1">
              <select value={startH} onChange={(e) => setStartH(+e.target.value)} className="flex-1 py-2 px-2.5 rounded-lg border border-portal-border text-sm bg-white outline-none">
                {hours.map((h) => <option key={h} value={h}>{fmt(h, 0)}</option>)}
              </select>
              <select value={startM} onChange={(e) => setStartM(+e.target.value)} className="w-[70px] py-2 px-2.5 rounded-lg border border-portal-border text-sm bg-white outline-none">
                {[0, 15, 30, 45].map((m) => <option key={m} value={m}>:{String(m).padStart(2, "0")}</option>)}
              </select>
            </div>
          </div>
          <div className="flex-1">
            <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">Varighet</div>
            <div className="flex flex-wrap gap-1">
              {durations.map((d) => (
                <button key={d} onClick={() => setDur(d)}
                  ...
                  style={{
                    ...
                    background: dur === d ? "var(--color-primary-soft)" : "var(--color-portal-hover)",
                    color: dur === d ? "var(--color-primary)" : "var(--color-portal-secondary)",
                    border: dur === d ? "2px solid var(--color-primary)" : "2px solid transparent",
                  }}
                >{d}m</button>
              ))}
            </div>
      ...
      {!showAdd ? (
        <button onClick={() => setShowAdd(true)} className="w-full py-3.5 rounded-[14px] border-2 border-dashed border-portal-border bg-transparent cursor-pointer text-sm font-semibold text-portal-secondary mb-4 hover:bg-portal-hover transition-colors">+ Legg til ovelse</button>
      ) : (
      ...
      <button onClick={save}
        className="w-full py-3.5 rounded-[20px] bg-accent-cta text-accent-cta-text text-[15px] font-bold border-none cursor-pointer shadow-[0_2px_8px_rgba(0,88,64,.15)] hover:opacity-85 transition-opacity"
...
function Planner({ events: initialEvents, templates, startLive, onSaveEvent, onDeleteEvent, onMoveEvent, onResizeEvent }: {
  ...
  return (
    <div className="flex h-[calc(100vh-53px)] overflow-hidden">
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-2.5 border-b border-black/4 bg-white flex-shrink-0">
          <div className="flex items-center gap-2">
            <button onClick={() => nav(-1)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-portal-text border-none cursor-pointer">&#8592;</button>
            <button onClick={() => nav(1)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-portal-text border-none cursor-pointer">&#8594;</button>
            <button onClick={() => setBaseDate(new Date())} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-accent-cta text-accent-cta-text border-none cursor-pointer">I dag</button>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setPlanning({ date: dk(new Date()), startH: 9, startM: 0 })}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-[20px] text-xs font-semibold bg-primary/10 text-primary border-none cursor-pointer hover:bg-primary/15 transition-colors"
            >+ Planlegg okt</button>
            <div className="flex gap-1 bg-portal-hover rounded-lg p-[3px]">
              {([["day", "Dag"], ["week", "Uke"], ["month", "Maned"]] as const).map(([k, l]) => (
                <button key={k} onClick={() => setView(k)} className="py-1.5 px-3.5 rounded-md text-xs font-semibold cursor-pointer border-none transition-all"
                  style={{ background: view === k ? "white" : "transparent", color: view === k ? "var(--color-portal-text)" : "var(--color-portal-secondary)", boxShadow: view === k ? "0 1px 2px rgba(0,0,0,.06)" : "none" }}
                >{l}</button>
              ))}
            </div>
          </div>
        ...
        {view === "month" ? (
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Day headers */}
            <div className="flex bg-white border-b border-portal-border flex-shrink-0">
                ...
                {dates.map((d, i) => {
                  ...
                  return (
                    <div key={i} className={`flex-1 text-center py-1.5 ${i > 0 ? "border-l border-black/4" : ""}`}>
                      <div className={`text-[10px] font-semibold uppercase tracking-[0.06em] ${td ? "text-primary" : "text-portal-secondary"}`}>{DN[di]}</div>
                      <div className={`text-lg font-bold w-7 h-7 rounded-full inline-flex items-center justify-center mt-0.5 tabular-nums ${td ? "bg-primary text-white" : "text-portal-text"}`}>{d.getDate()}</div>
                    </div>
                  );
                })}
              </div>
                ...
                <div className="w-[50px] flex-shrink-0">
                  {Array.from({ length: TH }, (_, i) => (
                    <div key={i} className="flex items-start justify-end pr-1.5" style={{ height: HH }}>
                      <span className="text-[10px] text-portal-muted tabular-nums -translate-y-1.5">{fmt(SH + i, 0)}</span>
                    </div>
                  ))}
                </div>
                        ...
                        {dayEv.map((ev) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {h > 28 && <div className="text-[9px] text-portal-secondary tabular-nums">{fmt(ev.startH, ev.startM)}-{fmt(et.h, et.m)}</div>}
      ...
      {/* SIDEBAR */}
      <div className="w-[260px] border-l border-portal-border bg-white overflow-y-auto p-3.5 flex flex-col gap-3.5 flex-shrink-0">
        {ev && (() => {
          const et = eTime(ev);
          const exCount = ev.exercises?.length || 0;
          return (
            <div className="bg-portal-bg rounded-xl p-3" style={{ border: `2px solid ${focColor(ev.focus)}30` }}>
              <div className="text-sm font-bold">{ev.title}</div>
              <div className="text-[11px] text-portal-secondary mt-0.5">{fmt(ev.startH, ev.startM)} - {fmt(et.h, et.m)} · {ev.dur} min</div>
              {exCount > 0 && (
                <div className="mt-2">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">{exCount} ovelser planlagt</div>
                  {ev.exercises.map((ex) => (
              ...
              <div className="flex gap-1.5 mt-2.5 flex-wrap">
                <button onClick={() => startLive(ev)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-accent-cta text-accent-cta-text border-none cursor-pointer">Start</button>
                <button onClick={() => setPlanning(ev.id)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-primary/10 text-primary border-none cursor-pointer">Rediger</button>
                <button onClick={() => updateEvent(ev.id, { done: !ev.done })}
                  className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold border-none cursor-pointer"
                  style={{ background: ev.done ? "var(--color-portal-hover)" : "var(--color-success-light)", color: ev.done ? "var(--color-portal-secondary)" : "var(--color-success)" }}
                >{ev.done ? "Angre" : "Fullfort"}</button>
                <button onClick={() => removeEvent(ev.id)} className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[20px] text-xs font-semibold bg-portal-hover text-error border-none cursor-pointer">Slett</button>
              </div>
            </div>
          );
        })()}
        ...
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">Standard okter</div>
          {(templates.length > 0 ? templates : DEFAULT_TEMPLATES).map((t) => (
            <div key={t.id} draggable onDragStart={(e) => e.dataTransfer.setData("template", JSON.stringify(t))}
              className="py-2 px-2.5 rounded-lg bg-portal-bg border border-black/4 cursor-grab mb-1 hover:bg-portal-hover transition-colors"
            >
        ...
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">Pyramiden</div>
  ...
  return (
    <div className="flex-1 p-4 overflow-y-auto">
      <div className="grid grid-cols-7 bg-white rounded-[14px] overflow-hidden border border-black/4">
        {DN.map((d) => (
          <div key={d} className="py-2 text-center text-[10px] font-semibold text-portal-secondary uppercase border-b border-black/4">{d}</div>
        ))}
        {cells.map((cell, i) => {
          ...
          return (
            <div key={i} onClick={() => { setBaseDate(cell.date); setView("day"); }}
              className="min-h-[80px] p-1.5 cursor-pointer hover:bg-portal-hover/50 transition-colors"
              style={{
                borderRight: (i + 1) % 7 ? "1px solid rgba(0,0,0,0.04)" : "none",
                borderBottom: "1px solid rgba(0,0,0,0.04)",
                background: td ? "var(--color-primary-soft)" : "transparent",
                opacity: cell.inM ? 1 : 0.35,
              }}
            >
              <div className={`text-xs w-6 h-6 rounded-full inline-flex items-center justify-center tabular-nums ${td ? "font-bold bg-primary text-white" : "font-medium text-portal-text"}`}>{cell.date.getDate()}</div>
              {de.slice(0, 2).map((ev) => (
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[{ l: "SNITT SCORE", v: "78.4" }, { l: "HANDICAP", v: "6.3" }, { l: "RUNDER", v: "14" }, { l: "SG TOTAL", v: "+1.8" }].map((k) => (
          <div key={k.l} className="bg-white rounded-xl shadow-card p-5">
            <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-1.5">{k.l}</div>
            <div className="text-4xl font-bold tabular-nums text-primary leading-none">{k.v}</div>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-card p-5">
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-3">Strokes Gained</div>
          {([["Tee", 0.6], ["Approach", 0.8], ["Short Game", -0.2], ["Putting", 0.6]] as const).map(([l, v]) => {
            ...
            return (
              <div key={l} className="mb-2.5">
                ...
                <div className="h-[7px] bg-portal-hover rounded-full relative overflow-hidden">
                  {v >= 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-card p-5">
          <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-3">Score-trend</div>
          <Spark data={[82, 80, 79, 81, 78, 77, 80, 78, 76, 79, 78, 77]} w={440} h={80} color="#005840" />
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-card p-5">
        <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-portal-muted mb-3">HCP 12 mnd</div>
        <Spark data={[8.1, 7.8, 7.5, 7.6, 7.2, 7.0, 6.9, 7.1, 6.8, 6.5, 6.4, 6.3]} w={1100} h={60} color="#2A7D5A" />
      </div>
    </div>
  );
}

function Spark({ data, w = 160, h = 40, color = "#005840" }: { data: number[]; w?: number; h?: number; color?: string }) {
...
export function TrainingPlannerV2({
  ...
  return (
    <div className="bg-portal-bg min-h-screen text-portal-text text-sm font-sans">
      {/* Tab bar (integrated with portal design) */}
      <div className="flex items-center justify-between px-6 py-2.5 bg-white border-b border-portal-border">
        <div className="flex gap-1 bg-portal-hover rounded-[10px] p-[3px]">
          {([["planner", "Planlegger"], ["analyse", "Analyse"]] as const).map(([k, l]) => (
            <button key={k} onClick={() => setTab(k)}
              ...
              style={{
                ...
                color: tab === k ? "var(--color-portal-text)" : "var(--color-portal-secondary)",
        ...
        {!liveSession ? (
          <button onClick={() => startLive(null)}
            className="inline-flex items-center gap-2 px-5 py-2 rounded-[20px] text-[13px] font-bold bg-accent-cta text-accent-cta-text border-none cursor-pointer hover:opacity-85 transition-opacity"
          >Start okt</button>
        ) : (
          <button onClick={() => setTab("live")}
            className="inline-flex items-center gap-2 px-5 py-2 rounded-[20px] text-[13px] font-bold bg-error text-white border-none cursor-pointer"
          >
