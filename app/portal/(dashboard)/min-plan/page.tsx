...
import { MinPlanClient } from "./min-plan-client";
...
export default async function MinPlanPage() {
  ...
  const serializedForecast = forecast
    ? {
        ...
        actualOutcomeMeasuredAt: forecast.actualOutcomeMeasuredAt?.toISOString() ?? null,
        deltaAllocationJson: (forecast.deltaAllocationJson ?? {}) as Record<string, number>,
        hoursPerCategoryJson: (forecast.hoursPerCategoryJson ?? {}) as Record<
          string,
          { hours: number; ci95Low: number; ci95High: number }
        >,
        techTactMentalPhysJson: (forecast.techTactMentalPhysJson ?? {}) as Record<
          string,
        ...
        confidenceInterval95: (forecast.confidenceInterval95 ?? [0, 0]) as [number, number],
      }
    : null;

  return <MinPlanClient forecast={serializedForecast} userName={user.name} />;
}
