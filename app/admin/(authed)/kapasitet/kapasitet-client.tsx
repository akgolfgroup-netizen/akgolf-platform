  ...
  return (
    <>
      ...
      <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {coaches.map((coach) => {
                ...
                return (
                  <div
                    key={coach.id}
                    className="flex flex-col items-center p-4 rounded-lg bg-[#F5F8F7]"
                  >
                    <AdminGauge
                      value={coachPct}
                      size={160}
        ...
        {/* Detail table */}
        <div>
          <h3 className="text-lg font-semibold text-[#0A1F18] mb-3">
            Detaljer per coach
          </h3>
          <AdminDataTable
        ...
        <div className="bg-white border border-[#D5DFDB] rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {emptySlotsList.length === 0 ? (
              <p className="text-sm text-[#7A8C85] py-4 text-center col-span-full">
              ...
              emptySlotsList.slice(0, 9).map((slot, i) => (
                <div
                  key={i}
                  className="p-3 bg-[#F5F8F7] rounded-lg flex items-center justify-between hover:bg-[#ECF0EF] transition-colors cursor-pointer"
                >
                  <div>
                    <div className="text-sm font-medium text-[#0A1F18] capitalize">
                      {slot.day}
                    </div>
                  ...
                  <span className="p-1.5 text-[#7A8C85]">
          ...
          {emptySlotsList.length > 9 && (
            <p className="text-xs text-[#7A8C85] mt-2 text-center">
        ...
        <div className="bg-white border border-[#D5DFDB] rounded-xl p-6">
          <div className="flex items-center gap-2 mb-4">
            <Sparkles className="w-5 h-5 text-[#5A6E66]" />
            <h3 className="text-lg font-semibold text-[#0A1F18]">
              AI-anbefalinger
            </h3>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {recommendations.map((rec) => (
              <div
                key={rec.title}
                className="p-4 bg-[#F5F8F7] rounded-lg border border-[#D5DFDB] hover:border-[#A5B2AD] transition-colors"
              >
                ...
                <p className="text-xs text-[#7A8C85] mb-3 leading-relaxed">
                  {rec.description}
                </p>
                <div className="flex items-center justify-between pt-3 border-t border-[#D5DFDB]">
                  ...
                  <span
                    className="text-xs text-[#7A8C85] cursor-not-allowed"
                    title="Kommer snart"
                  >
                    Kommer snart
                  </span>
        ...
        <div className="bg-white border border-[#D5DFDB] rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <p className="text-xs text-[#7A8C85]">Inntekt</p>
              <p className="text-lg font-bold text-[#0A1F18] tabular-nums">
                {formatKr(data.monthlyTotal.revenue)}
              </p>
            </div>
            <div>
              <p className="text-xs text-[#7A8C85]">Maks potensiell</p>
              <p className="text-lg font-bold text-[#0A1F18] tabular-nums">
                {formatKr(data.monthlyTotal.maxRevenue)}
              </p>
            </div>
            <div>
              <p className="text-xs text-[#7A8C85]">Bookinger</p>
              <p className="text-lg font-bold text-[#0A1F18] tabular-nums">
