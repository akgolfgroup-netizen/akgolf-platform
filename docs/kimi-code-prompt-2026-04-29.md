...
Fire features beskrevet under. Du kan jobbe på én av gangen, eller flere parallelt om de ikke berører samme filer. Hver feature har:

- **Mål** (hva spilleren får)
- **Eksisterende kode** (filer du skal bygge på)
...
1. **Brand Guide V2.0:** `--color-primary: #005840`, `--color-accent: #D1F843`, `--color-surface: #F4F6F4`, `--color-card: #FFFFFF`, `--color-ink: #0A1F18`, `--color-sidebar: #0F1F18`. Se `.claude/rules/design-system.md`.
2. **Fonter:** Inter Tight (headlines), Inter (body), JetBrains Mono (eyebrows + tall).
3. **Ikoner:** `lucide-react` KUN. Aldri Material Symbols, aldri emojier.
4. **Maks 300 linjer per fil** — splitt i flere komponenter ved behov.
5. **TypeScript strict** — aldri `any` uten kommentar som forklarer hvorfor.
6. **Server Components der mulig** — kun `"use client"` der reell interaktivitet.
7. **Norsk bokmål** for all brukervendt tekst (sjekk `.claude/rules/sprak.md`).
8. **Spiller, aldri elev** — terminologi-lås.
9. **Mobil-først** — alle nye sider skal fungere godt på iPhone 14 Pro / Pixel 7. Touch-targets minst 44px.
...
### Commit-format:
```
feat(F1/F2/F3/F4): kort beskrivelse
```
Bruk `git add <spesifikke filer>` (ALDRI `git add -A`)

### Ikke jobb på samme fil samtidig som Claude.
Sjekk `git log` for siste commits før du starter.
...
Backend bruker Anthropic Claude Vision-modell for å lese bildet, parser dataen strukturert, og lagrer til spillerens profil. Frontend viser:
...
- Forhåndsvisning av bilde før upload
...
- Suksess: "X slag importert" + lenke til øktdetalj
- Feilhåndtering: vis konfidens-score + mulighet for manuell korreksjon
...
- Hvert slag = sirkel (størrelse = ball speed, farge = kolle)
...
- [ ] CSV-import fortsetter å fungere
...
Spilleren fører slag-for-slag-statistikk på banen via mobil:

- Hvert slag: kolle, distanse, lie (fairway/rough/bunker/green)
- Etter runden: full Strokes Gained-analyse + scorekort

**Fase 1 (DENNE):** Manuell føring uten kart. Rask, mobilvennlig flyt.  
...
- [ ] Spiller kan starte runde på < 30 sek
- [ ] Logge ett slag tar < 10 sek på mobil
...
Visuell baneguide som UpGame Pro: spilleren ser hvert hull på kart, GPS-tracking viser hvor de står, automatisk lie-detection, og slag visualiseres på kartet.

**Integrert med F2 (live-runde):**

- F2 = manuell føring uten kart
- F3 = kart-basert føring der GPS gjør det raskere

Spilleren skal kunne:

- Se hver hull-layout (tee → fairway → green)
- Sette markør på kartet for "her står jeg"
- Tappe på kartet for "her landet ballen"
- Auto-foreslår kolle basert på GPS-distanse
...
- Støtte for satellite + custom layers
- Aktiv React-integrasjon (react-map-gl)

Alternativ: Leaflet + OpenStreetMap — gratis, men mindre presist.

**Anbefaling:** Start med Mapbox. Bruk personal access token i MVP, bytt til Mapbox Pro (~$100/mnd) når dere skalerer.
...
export function detectLieFromGps(
  ...
  // Bruk turf.js (point-in-polygon) for å sjekke hvilket area
  // 1. Sjekk om i green
  // 2. Sjekk om i bunker
...
For nå: manuell import via admin-side. Senere kan dette automatiseres via Google Maps API for å auto-generere bane-overlay.
...
- Kart sentreres på current hull (`Hole.latitude`, `longitude`)
- Vis tee-marker, green-marker
- GPS-prikk (med `navigator.geolocation`)
- Tap-for-å-sette-mål-marker (eller auto-bruk GPS for "fra-posisjon")
- Auto-distanse-beregning (`turf.distance`)
- Auto-lie-detection
- Kolle-forslag basert på distanse + ClubInBag

#### 3. Admin: bane-import-side: `/admin/baner/[id]/import`

For Anders/coaches å laste opp GeoJSON for nye baner:

- Drag-drop GeoJSON-fil
- Forhåndsvisning på kart
...
- `components/portal/round/shot-marker.tsx` (NY) — slag-markør
- `components/portal/round/club-suggester.tsx` (NY) — AI-kolle-forslag

### Akseptansekriterier (F3)

- [ ] Mapbox-kart vises riktig på mobil
- [ ] GPS-prikk oppdateres i sanntid
- [ ] Tap på kartet setter mål-marker
- [ ] Auto-distanse vises korrekt (testet mot manuell måling)
- [ ] Auto-lie-detection fungerer for alle 5 lie-typer
- [ ] Kolle-forslag basert på distanse + `ClubInBag.avgCarryMeters`
- [ ] Off-line: kart fungerer uten internett (vector-tiles cached)
- [ ] Coach kan se slag-markøringer i `/admin/elever?id=<userId>` for hver runde

### Kostnads-vurdering

- Mapbox personal access token: gratis opp til 50 000 map-loads/mnd
- Mapbox Pro: $100/mnd ved større volum
- Custom satellite-bilder: $0.50/km² for høy-oppløsning av spesifikke baner

**Anbefaling:** Start gratis. Test med 5-10 spillere på Gamle Fredrikstad GK. Bytt til Pro når dere har > 50 aktive brukere.
...
- Treningsområde (TEE, INN150, CHIP, PUTT0-3, etc.)
...
├──────────────────────────────────────────────────────┤
│ Filtre:                                              │
│ [ Pyramide ▼ TEK,SLAG ]  [ Område ▼ TEE ]           │
│ [ L-fase ▼ L-AUTO ]  [ CS ▼ CS80+ ]                  │
│ [ Miljø ▼ M3-M5 ]  [ Press ▼ alle ]                  │
│ [ Periode: siste 90 dager ▼ ]                        │
│ [ Tøm filtre ]  [ Sammenlign ]                       │
├──────────────────────────────────────────────────────┤
│ Resultat: 23 økter · 14 timer · 4 uker              │
│                                                      │
│ ┌─ Per pyramide ──┐  ┌─ Per område ──┐              │
│ │ TEK: 8 (5t)     │  │ TEE: 12 (7t)  │              │
...
- "Sammenlign"-toggle åpner 2-kolonne (filter A vs B)
- Resultat oppdateres på < 500ms
...
- [ ] Spiller kan filtrere på alle 6 dimensjoner samtidig
- [ ] Filtre persisteres i URL og kan deles
- [ ] Aggregat oppdateres på < 500ms
- [ ] Trend-graf viser 12 uker
- [ ] Sammenlign-modus viser to filter side-om-side
- [ ] Coach kan kjøre samme analyse for spiller
- [ ] Mobil: filter-bar collapser til "Filtre (3 aktive)" pille
- [ ] Tom-state: "Ingen økter matcher — prøv å fjerne noen filtre"
...
3. Les `.claude/rules/sprak.md` (terminologi-lås)
4. Les `.claude/rules/gotchas.md` (kjente feller)
5. Velg én feature og følg backend-krav → frontend-krav → akseptansekriterier
...
Hvis du gjør ÉN av gangen:

1. **F1 TrackMan** (3-4 dager) — mest gjenbruk, lavest risiko
2. **F2 Stats manuell** (3 dager) — bygger på eksisterende Round-modell
...
- **D:** 20 fastsatte golftester utfør-flow
...
Anders er CEO, ikke programmerer. Lag konkrete forslag, ikke abstrakte alternativer. Hvis du er usikker — bygg det enkleste som dekker akseptansekriteriene, og dokumenter avgjørelser i kommentarer.

For state-håndtering: foretrekk URL-state over React state der mulig (`searchParams`). Det gjør lenker delbare og state persistent.
