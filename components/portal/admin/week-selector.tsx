...
export function WeekSelector({ selectedWeek, onWeekChange }: WeekSelectorProps) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 bg-white rounded-xl p-3 border border-[var(--color-grey-200)]">
      {/* Previous Week */}
      <motion.button
      ...
      <div className="flex-1 text-center">
        <div className="flex items-center justify-center gap-2">
          <CalendarDays className="w-4 h-4 text-[var(--color-grey-400)]" />
          <span className="font-semibold text-[var(--color-grey-900)]">
            Uke {weekNumber}
          </span>
          {isCurrentWeek && (
            <span className="text-xs bg-[var(--color-grey-900)] text-white px-2 py-0.5 rounded-full">
              Denne uken
            </span>
          )}
        </div>
      ...
      {!isCurrentWeek && (
        <motion.button
          onClick={goToToday}
          className="px-3 py-1.5 text-xs font-medium text-[var(--color-grey-900)] bg-[var(--color-grey-100)] rounded-full hover:bg-[var(--color-grey-200)] transition-colors"
