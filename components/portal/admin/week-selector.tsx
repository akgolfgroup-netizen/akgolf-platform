...
export function WeekSelector({ selectedWeek, onWeekChange }: WeekSelectorProps) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 bg-white rounded-xl p-3 border border-[#D5DFDB]">
      {/* Previous Week */}
      <motion.button
      ...
      >
        <ChevronLeft className="w-5 h-5 text-[#7A8C85]" />
      </motion.button>

      {/* Week Display */}
      <div className="flex-1 text-center">
        <div className="flex items-center justify-center gap-2">
          <CalendarDays className="w-4 h-4 text-[#7A8C85]" />
          <span className="font-semibold text-[#0A1F18]">
            Uke {weekNumber}
          </span>
          {isCurrentWeek && (
            <span className="text-xs bg-[#0A1F18] text-white px-2 py-0.5 rounded-full">
              Denne uken
            </span>
          )}
        </div>
      ...
      {!isCurrentWeek && (
        <motion.button
          onClick={goToToday}
          className="px-3 py-1.5 text-xs font-medium text-[#0A1F18] bg-[#ECF0EF] rounded-full hover:bg-[#D5DFDB] transition-colors"
