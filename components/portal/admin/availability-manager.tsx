    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="relative">
          <div className="flex items-center gap-3 px-4 py-3 bg-white rounded-xl border border-grey-200 shadow-sm">
            ...
            <select
              ...
              className="bg-transparent text-sm font-medium text-black pr-6 appearance-none cursor-pointer focus:outline-none"
            >
            ...
            <ChevronDown className="w-4 h-4 text-grey-400 absolute right-4" />
          </div>
        </div>
        ...
        <div className="flex bg-grey-100 rounded-xl p-1">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-[background-color,color] duration-300 ${ activeTab === "schedule" ? "bg-white text-black shadow-sm" : "text-grey-400 hover:text-text" }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-[background-color,color] duration-300 ${ activeTab === "blocked" ? "bg-white text-black shadow-sm" : "text-grey-400 hover:text-text" }`}
          >
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          >
            <motion.div
              className="w-8 h-8 border-3 border-black border-t-transparent rounded-full"
              animate={{ rotate: 360 }}
              transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
            />
            <p className="mt-4 text-sm text-grey-400">Laster tilgjengelighet...</p>
          </motion.div>
        ) : activeTab === "schedule" ? (
          <motion.div
            key="schedule"
              ...
              {blockedTimes.length === 0 ? (
                <div className="py-12 flex flex-col items-center justify-center">
                  <div className="w-12 h-12 rounded-full bg-grey-100 flex items-center justify-center mb-3">
                    <Ban className="w-6 h-6 text-grey-400" />
                  </div>
                ...
                <ul className="divide-y divide-grey-100">
                  {blockedTimes.map((bt, index) => (
                    <motion.li
                      ...
                      className="flex items-center justify-between px-6 py-4 hover:bg-grey-100 transition-colors"
                    >
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {bt.reason && (
                              <>
                                ...
                                <span className="text-xs text-grey-400">{bt.reason}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      <motion.button
                        ...
                        className="p-2 rounded-lg hover:bg-error/5 text-grey-400 hover:text-error transition-colors"
