...
export function NotificationPanel({ isOpen, onClose }: NotificationPanelProps) {
  ...
  const getNotificationIcon = (type: string, adminType: string | null) => {
    switch (adminType) {
      ...
      case "urgent":
        return <AlertCircle className="w-4 h-4 text-[var(--hg-error)]" />;
      case "system":
      default:
    ...
    const content = (
      <div
        className={cn(
          ...
          notification.read
            ? "bg-transparent hover:bg-[var(--hg-surface-raised)]"
            : "bg-[var(--hg-primary)]/5 hover:bg-[var(--hg-primary)]/10 border-l-2 border-[var(--hg-primary)]"
        )}
      ...
      >
        <div className={cn(
          ...
          notification.read
            ...
            : "bg-[var(--hg-primary)]/10 text-[var(--hg-primary)]"
        )}>
        ...
        <div className="flex-1 min-w-0">
          <p className={cn(
            ...
            notification.read
              ...
              : "text-[var(--hg-text)] font-medium"
          )}>
            {notification.title}
          </p>
          <p className="text-[11px] text-[var(--hg-text-muted)] mt-0.5 line-clamp-2">
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {notification.linkUrl && (
              <Link
                ...
                className="text-[10px] text-[var(--hg-primary)] hover:underline flex items-center gap-0.5"
              >
        ...
        {!notification.read && (
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-[var(--hg-surface-raised)] transition-colors flex-shrink-0 opacity-0 group-hover:opacity-100"
            title="Merk som lest"
          >
            {markingRead.includes(notification.id) ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin text-[var(--hg-text-muted)]" />
            ) : (
              <Check className="w-3.5 h-3.5 text-[var(--hg-text-muted)]" />
            )}
          </button>
        )}
      </div>
    ...
    <button
      ...
      className={cn(
        ...
        activeFilter === filter
          ...
          : "bg-[var(--hg-surface-raised)] text-[var(--hg-text-muted)] hover:text-[var(--hg-text)]"
      )}
    >
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-full sm:w-[400px] bg-[var(--hg-surface)] border-l border-[var(--hg-border)] z-50 flex flex-col"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--hg-border)]">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <Bell className="w-5 h-5 text-[var(--hg-text)]" />
                  {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-[var(--hg-error)] text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                      {unreadCount > 9 ? "9+" : unreadCount}
                    </span>
                  )}
                </div>
                <div>
                  <h2 className="text-sm font-semibold text-[var(--hg-text)]">Notifikasjoner</h2>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-2 rounded-lg text-[var(--hg-text-muted)] hover:text-[var(--hg-text)] hover:bg-[var(--hg-surface-raised)] transition-colors"
            ...
            {/* Filters */}
            <div className="flex items-center gap-2 px-4 py-2 border-b border-[var(--hg-border)] overflow-x-auto">
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoading && notifications.length === 0 ? (
                <div className="flex items-center justify-center h-32">
                  <Loader2 className="w-6 h-6 animate-spin text-[var(--hg-text-muted)]" />
                </div>
              ) : notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center px-4">
                  <Bell className="w-12 h-12 text-[var(--hg-text-muted)] mb-3" />
                  <p className="text-sm text-[var(--hg-text-muted)]">Ingen notifikasjoner</p>
                ...
                <div className="p-2 space-y-1">
                  ...
                  {grouped?.today && grouped.today.length > 0 && (
                    <>
                      <div className="px-2 py-1 text-[10px] font-medium text-[var(--hg-text-muted)] uppercase tracking-wider">
                        I dag
                      </div>
                      {grouped.today.map((n) => (
                  ...
                  {grouped?.yesterday && grouped.yesterday.length > 0 && (
                    <>
                      <div className="px-2 py-1 mt-3 text-[10px] font-medium text-[var(--hg-text-muted)] uppercase tracking-wider">
                        I går
                      </div>
                  ...
                  {hasMore && (
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full py-2 text-[11px] text-[var(--hg-primary)] hover:text-[var(--hg-primary)]/80 transition-colors"
                    >
