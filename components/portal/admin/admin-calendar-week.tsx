...
export function AdminCalendarWeek({ date, bookings, onSelectBooking }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-grey-200 bg-white backdrop-blur-md overflow-hidden">
      {/* Day headers */}
      <div className="grid border-b border-grey-200" style={{ gridTemplateColumns: "56px repeat(7, 1fr)" }}>
        <div /> {/* spacer for time column */}
        {days.map((day) => (
          <div
            key={day.toISOString()}
            className={`text-center py-3 border-l border-grey-200 ${ isToday(day) ? "bg-black/10" : "" }`}
          >
            <p className="text-[10px] uppercase text-grey-400 font-medium">
              {format(day, "EEE", { locale: nb })}
            </p>
            <p
              className={`text-sm font-semibold mt-0.5 ${ isToday(day) ? "bg-black text-white w-7 h-7 rounded-full flex items-center justify-center mx-auto" : "text-black" }`}
            >
              {format(day, "d")}
            </p>
          </div>
      ...
      >
        ...
        <div className="relative" style={{ height: HOURS.length * SLOT_HEIGHT }}>
          {HOURS.map((hour) => (
            ...
            >
              <span className="text-[10px] text-grey-400 px-2 -translate-y-1/2 block">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={`relative border-l border-grey-200 ${ isToday(day) ? "bg-black/5" : "" }`}
            ...
            >
              ...
              {HOURS.map((hour) => (
                <div
                  key={hour}
                  className="absolute left-0 right-0 border-t border-grey-200"
                  style={{ top: (hour - 8) * SLOT_HEIGHT }}
                />
              ))}
