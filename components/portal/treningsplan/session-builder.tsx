    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          {/* Panel */}
          <motion.div
            className="fixed right-0 top-0 bottom-0 w-[520px] bg-white border-l border-[#E8E8ED] z-50 flex flex-col shadow-2xl"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E8ED]">
              <div>
                <h2 className="text-lg font-semibold text-[#1D1D1F]">Okt-builder</h2>
                {dayLabel && (
              ...
              <div className="flex items-center gap-2">
                {/* Mode toggle */}
                <div className="flex border border-[#E8E8ED] rounded-lg overflow-hidden">
                  <button
                    onClick={() => setMode("quick")}
                    className={`text-[10px] font-semibold px-3 py-1.5 transition-colors ${
                      mode === "quick" ? "bg-[#1D1D1F] text-white" : "text-[#86868B] hover:bg-[#F5F5F7]"
                    }`}
                  >
                    Quick
                  </button>
                  <button
                    ...
                    className={`text-[10px] font-semibold px-3 py-1.5 transition-colors ${
                      mode === "full" ? "bg-[#1D1D1F] text-white" : "text-[#86868B] hover:bg-[#F5F5F7]"
                    }`}
                  >
                    Full
                  </button>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="bg-[#F5F5F7] rounded-xl p-3 flex items-center justify-between">
                <div>
                  <span className="text-[9px] font-bold uppercase tracking-wide text-[#86868B] block">
                    OKT-ID (auto)
                  </span>
                  <span className="text-[11px] font-mono text-[#1D1D1F] mt-0.5 block break-all">
                    {sessionId}
                  </span>
                </div>
              ...
              <div className="pt-2">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button className="flex items-center gap-1 text-[10px] font-semibold text-[#1D1D1F] hover:text-[#2D6A4F] transition-colors">
                ...
                <div className="space-y-1.5">
                  {exercises.map((ex, idx) => (
                    <div
                      key={ex.id}
                      className="flex items-center gap-2 p-3 rounded-xl bg-[#F5F5F7] hover:bg-[#E8E8ED]/60 group transition-colors"
                    >
                      <GripVertical className="w-3 h-3 text-[#D2D2D7] cursor-grab flex-shrink-0" />
                      <span className="text-xs font-semibold text-[#86868B] w-5">{idx + 1}.</span>
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-semibold text-[#1D1D1F] block truncate">
                          {ex.name}
                        </span>
                        <span className="text-[10px] text-[#86868B]">{ex.meta}</span>
                      </div>
                ...
                <FormField label="Gjenta">
                  <div className="flex gap-1">
                    {["Ikke gjenta", "Hver uke", "Custom"].map((opt) => (
                      <button
                        key={opt}
                        className="text-[10px] font-semibold px-3 py-1.5 rounded-lg border border-[#E8E8ED] text-[#86868B] hover:bg-[#F5F5F7] transition-colors"
                      >
            ...
            <div className="px-6 py-4 border-t border-[#E8E8ED] flex justify-between">
              <button
                onClick={onClose}
                className="px-5 py-2.5 text-sm font-semibold text-[#86868B] hover:text-[#1D1D1F] transition-colors"
              >
                Avbryt
              </button>
              <button className="px-6 py-2.5 rounded-full bg-[#1D1D1F] text-white text-sm font-semibold hover:bg-[#3A3A3C] transition-colors">
                Lagre okt
              </button>
            </div>
          </motion.div>
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[11px] font-semibold uppercase tracking-[0.05em] text-[#86868B] mb-1.5">
        {label}
      </label>
      {children}
    </div>
