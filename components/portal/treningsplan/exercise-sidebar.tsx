...
export function ExerciseSidebar() {
  ...
  return (
    <div className="w-[300px] bg-white border-l border-grey-200 flex flex-col h-full overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-grey-200">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-xs bg-surface border border-grey-200 rounded-lg focus:outline-none focus:border-black focus:ring-1 focus:ring-black/10 transition-colors"
          />
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {filtered.length === 0 ? (
          ...
          filtered.map((ex) => (
            <div
              key={ex.id}
              className="flex items-start gap-2 p-3 rounded-xl bg-surface border border-transparent hover:border-grey-300 hover:bg-white cursor-grab active:cursor-grabbing transition-colors group"
            >
              <GripVertical className="w-3.5 h-3.5 text-muted mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-grey-400 mt-0.5 ml-3.5">
                  {ex.area} {ex.meta ? `\u00B7 ${ex.meta}` : ""} {ex.mLevel > 0 ? `\u00B7 M${ex.mLevel}` : ""}
                </p>
              </div>
              <span className="text-[10px] font-semibold text-grey-400 tabular-nums flex-shrink-0">
      ...
      {/* AI suggestion */}
      <div className="p-4 border-t border-grey-200 bg-[var(--portal-ai-light)]">
        <div className="flex items-center gap-2 mb-2">
          <Sparkles className="w-3.5 h-3.5 text-ai" />
          <span className="text-[10px] font-bold uppercase tracking-wide text-ai-text">
            AI-forslag
          </span>
        </div>
        <p className="text-[11px] text-ai-text leading-relaxed mb-3">
          Naerspill er ditt svakeste omrade. Legg til 2 okter denne uken.
        </p>
        <button className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-ai text-white text-[10px] font-semibold hover:opacity-90 transition-opacity">
