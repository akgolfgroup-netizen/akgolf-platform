  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={null}>
        {activeItem && (
          <div className="flex items-center gap-2 p-3 rounded-xl bg-white border border-[#2D6A4F] shadow-[0_16px_48px_rgba(0,0,0,0.12)] w-[260px] opacity-90">
            <GripVertical className="w-3.5 h-3.5 text-[#86868B] flex-shrink-0" />
            <div className={`w-1.5 h-1.5 rounded-full ${PYRAMID_DOT[activeItem.pyramid] || "bg-[#86868B]"}`} />
            <span className="text-xs font-semibold text-[#1D1D1F] truncate flex-1">
              {activeItem.name}
            </span>
            <span className="flex items-center gap-1 text-[10px] text-[#86868B] tabular-nums">
