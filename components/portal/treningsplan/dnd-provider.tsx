  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={null}>
        {activeItem && (
          <div className="flex items-center gap-2 p-3 rounded-xl bg-white border border-primary shadow-[0_16px_48px_rgba(0,0,0,0.12)] w-[260px] opacity-90">
            <GripVertical className="w-3.5 h-3.5 text-grey-400 flex-shrink-0" />
            <div className={`w-1.5 h-1.5 rounded-full ${PYRAMID_DOT[activeItem.pyramid] || "bg-grey-400"}`} />
            <span className="text-xs font-semibold text-black truncate flex-1">
              {activeItem.name}
            </span>
            <span className="flex items-center gap-1 text-[10px] text-grey-400 tabular-nums">
