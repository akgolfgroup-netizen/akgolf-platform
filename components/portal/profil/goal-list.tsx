...
const CATEGORY_CONFIG: Record<GoalCategory, { label: string; color: string }> = {
  ...
  PHYSICAL: { label: "Fysisk", color: "var(--color-success)" },
  MENTAL: { label: "Mental", color: "var(--color-ai)" },
  TOURNAMENT: { label: "Turnering", color: "var(--color-info)" },
  PROCESS: { label: "Prosess", color: "var(--color-warning)" },
};
  ...
  return (
    ...
    >
      <button
        ...
        style={{
          borderColor: isCompleted ? "var(--color-success)" : "var(--color-grey-200)",
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
        ...
        <button onClick={onDelete} className="p-1 rounded hover:bg-[var(--color-error)]/10 transition-colors">
          <Trash2 className="w-3 h-3 text-[var(--color-error)]/50 hover:text-[var(--color-error)]" />
        </button>
      </div>
    </motion.div>
  );
