...
function StudentAvatar({ name }: { name: string }) {
  ...
  return (
    <div className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-sm font-semibold text-on-surface-variant">
      {initial}
    </div>
  );
}
    ...
    <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-surface transition-colors">
      <span
        className={cn(
          ...
          rank === 2 && "bg-surface-variant text-on-surface-variant",
          rank === 3 && "bg-[#CD7F32]/20 text-[#CD7F32]",
          rank > 3 && "text-on-surface-variant/60"
        )}
      >
        {rank}
      </span>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-on-surface-variant">HCP: {student.currentHcp}</p>
      </div>
  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-error-light/50 hover:bg-error-light transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-on-surface-variant">{student.lastActivity}</p>
    ...
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-semibold text-on-surface">Elev-progresjon</h2>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-on-surface-variant">Snitt HCP:</span>
          <span className="text-sm font-semibold text-on-surface tabular-nums">{currentHcp}</span>
          <span
            className={cn(
      ...
      {/* Top 5 Improvements */}
      <div className="mb-6">
        <h3 className="text-xs font-medium text-on-surface-variant uppercase tracking-wide mb-3">
          Topp 5 forbedringer
        </h3>
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      <div className="text-center p-3 rounded-lg bg-surface">
        ...
        <p className="text-[10px] text-on-surface-variant uppercase tracking-wide">Totalt</p>
      </div>
      <div className="text-center p-3 rounded-lg bg-surface">
        <p className="text-2xl font-bold text-on-surface tabular-nums">{averageHcp}</p>
        <p className="text-[10px] text-on-surface-variant uppercase tracking-wide">Snitt HCP</p>
      </div>
      <div className="text-center p-3 rounded-lg bg-surface">
        <p className="text-2xl font-bold text-on-surface tabular-nums">{activeThisWeek}</p>
        <p className="text-[10px] text-on-surface-variant uppercase tracking-wide">Aktive denne uke</p>
      </div>
    </div>
  );
}
