  ...
  return (
    <section className="w-section bg-surface">
      <div className="w-container">
        <RevealOnScroll>
          <SectionLabel>Hva vi gjør</SectionLabel>
        </RevealOnScroll>
        <RevealOnScroll>
          <h2 className="w-heading-lg mt-4 mb-4">
            Tre divisjoner. Ett mål.
          </h2>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {DIVISIONS.map((div) => (
            <StaggerItem key={div.id}>
              <Link
                href={div.href}
                className="group block bg-white rounded-2xl p-8 md:p-10 hover:shadow-card-hover transition-shadow duration-300"
              >
