...
export function registerBreakingPointTools(server) {
    server.registerTool('ak_bp_log', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_bp_log');
        return success(data);
    });
    server.registerTool('ak_bp_history', {
        ...
        if (params.bp_type)
            q = q.eq('bp_type', params.bp_type);
        const { data, error: err } = await q;
        if (err)
            return handleSupabaseError(err, 'ak_bp_history');
        const bps = data ?? [];
        const byType = {};
        for (const bp of bps) {
            if (!byType[bp.bp_type])
                byType[bp.bp_type] = [];
            byType[bp.bp_type].push(bp.threshold);
        }
    ...
    server.registerTool('ak_bp_progression', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_bp_progression');
        if (!bps?.length)
            return success({ player_id: params.player_id, total: 0, progression: {}, message: 'Ingen breaking points registrert.' });
        const extractNum = (t) => { const m = t.match(/\d+/); return m ? parseInt(m[0]) : 0; };
        const prog = {};
        for (const type of ['CS', 'M', 'PR']) {
            const tb = bps.filter(b => b.bp_type === type);
            if (!tb.length)
                continue;
            const fN = extractNum(tb[0].threshold);
            const lN = extractNum(tb[tb.length - 1].threshold);
...
//# sourceMappingURL=breaking-points.js.map