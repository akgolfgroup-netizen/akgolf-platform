...
export function registerTrackmanTools(server) {
    server.registerTool('ak_trackman_log', {
        ...
        if (sErr)
            return handleSupabaseError(sErr, 'ak_trackman_log (session)');
        ...
        if (shErr)
            return handleSupabaseError(shErr, 'ak_trackman_log (shots)');
    ...
    server.registerTool('ak_trackman_analyze', {
        ...
        if (!sessions?.length)
            return success({ player_id: params.player_id, period_days: params.days, total_shots: 0, total_sessions: 0, by_club: {}, message: `Ingen TrackMan-data siste ${params.days} dager.` });
        let q = sb.from('trackman_shots').select('*').in('trackman_session_id', sessions.map(s => s.id)).order('created_at');
        if (params.club)
            q = q.eq('club', params.club);
        const { data: shots, error: err } = await q;
        if (err)
            return handleSupabaseError(err, 'ak_trackman_analyze');
        if (!shots?.length)
            return success({ player_id: params.player_id, period_days: params.days, total_shots: 0, total_sessions: sessions.length, by_club: {}, message: 'Ingen slag med disse filtrene.' });
        const byClub = {};
        ...
        for (const [club, cs] of Object.entries(grouped)) {
            ...
            for (const f of fields) {
                if (params.metrics?.length && !params.metrics.includes(f))
                    continue;
                const vals = cs.map(s => s[f]).filter((v) => v != null);
                if (vals.length) {
                    const avg = vals.reduce((a, b) => a + b, 0) / vals.length;
                    byClub[club][`${f}_avg`] = Math.round(avg * 100) / 100;
                    byClub[club][`${f}_std`] = Math.round(Math.sqrt(vals.reduce((s, v) => s + (v - avg) ** 2, 0) / vals.length) * 100) / 100;
                }
            }
        }
...
//# sourceMappingURL=trackman.js.map