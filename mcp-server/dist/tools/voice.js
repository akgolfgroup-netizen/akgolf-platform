...
export function registerVoiceTools(server) {
    server.registerTool('ak_voice_save', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_voice_save');
        return success(data);
    });
    server.registerTool('ak_voice_search', {
        ...
        if (params.player_id)
            q = q.eq('player_id', params.player_id);
        if (params.query)
            q = q.or(`transcript.ilike.%${params.query}%,summary.ilike.%${params.query}%`);
        const { data, error: err } = await q;
        if (err)
            return handleSupabaseError(err, 'ak_voice_search');
