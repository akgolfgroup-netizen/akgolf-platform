...
import { DISTRIBUTION_TEMPLATES, HOURS_PER_WEEK } from '../constants.js';
import { z } from 'zod';
...
Returns:
  { player: {...}, training_last_7_days: { actual vs target distribution, gaps }, trackman, coaching_notes, latest_test, breaking_points, invariant_violations[] }`,
        ...
        const byPyr = {};
        for (const s of sl)
            byPyr[s.pyramid_level] = (byPyr[s.pyramid_level] ?? 0) + s.duration_minutes;
        ...
        const gaps = Object.entries(targetPct).filter(([k, v]) => Math.abs((actualPct[k] ?? 0) - v) > 10).map(([k, v]) => `${k}: ${actualPct[k] ?? 0}% vs mål ${v}%`);
        // TrackMan
        ...
        // Invariant check
        const violations = [];
        if (totalMin > 0 && (actualPct['TEK'] ?? 0) < 15)
            violations.push(`#1: TEK er ${actualPct['TEK'] ?? 0}% — minimum 15%`);
        const turnCount = sl.filter(s => s.pyramid_level === 'TURN').length;
        if (['H', 'I', 'J', 'K'].includes(cat) && turnCount > 1)
            violations.push(`#5: Kat ${cat} maks 1 turneringshelg, fant ${turnCount} TURN-økter`);
        return success({
    ...
    server.registerTool('ak_training_plan_save', {
        ...
        let tKey = `${cat}_${p}`;
        if (p === 'TURN')
            tKey = wt === 'TURNERINGSUKE' ? `${cat}_TURN_T` : `${cat}_TURN_TR`;
        ...
        if (err)
            return handleSupabaseError(err, 'ak_training_plan_save');
        return success(data);
    });
    server.registerTool('ak_training_plan_get', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_training_plan_get');
        if (!data?.length)
            return error('Ingen treningsplan funnet.', 'Bruk ak_training_analyze + ak_training_plan_save for å generere en.');
