...
export function registerPlayerTools(server) {
    server.registerTool('ak_player_create', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_player_create');
        return success(data);
    });
    server.registerTool('ak_player_get', {
        ...
        const sb = getSupabase();
        if (!params.player_id && !params.name)
            return error('Oppgi player_id eller name.', 'Bruk ak_player_list for å se alle spillere.');
        let q = sb.from('players').select('*');
        if (params.player_id)
            q = q.eq('id', params.player_id);
        else if (params.name)
            q = q.ilike('name', `%${params.name}%`);
        const { data, error: err } = await q;
        if (err)
            return handleSupabaseError(err, 'ak_player_get');
        if (!data?.length)
            return error(`Ingen spiller funnet${params.name ? ` med navn "${params.name}"` : ''}.`, 'Sjekk stavemåte eller bruk ak_player_list.');
    ...
    server.registerTool('ak_player_list', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_player_list');
        return success({ players: data ?? [] });
    });
    server.registerTool('ak_session_log', {
        ...
        if (err)
            return handleSupabaseError(err, 'ak_session_log');
        return success(data);
    });
    server.registerTool('ak_session_history', {
        ...
        const { data, error: err } = await q;
        if (err)
            return handleSupabaseError(err, 'ak_session_history');
        const sessions = data ?? [];
