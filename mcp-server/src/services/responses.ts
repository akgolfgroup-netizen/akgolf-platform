...
export function success(data: Record<string, unknown>, markdown?: string): ToolResponse {
...
export function error(message: string, suggestion?: string): ToolResponse {
...
export function handleSupabaseError(err: unknown, context: string): ToolResponse {
...
export function formatPlayerMarkdown(p: Record<string, unknown>): string {
  return `**${p.name}** (Kat ${p.category})\nHCP: ${p.handicap ?? '—'} | Snitt: ${p.avg_score ?? '—'} | Periode: ${p.current_period ?? '—'}`;
}

export function formatDrillMarkdown(d: Record<string, unknown>): string {
  return `**${d.name}** [${d.pyramid_level}/${d.difficulty}]\n${d.goal}\nVarighet: ${d.duration_minutes} min | Miljø: ${(d.environments as string[])?.join(', ')}`;
}

export function formatPaginationMarkdown(total: number, count: number, offset: number, hasMore: boolean): string {
