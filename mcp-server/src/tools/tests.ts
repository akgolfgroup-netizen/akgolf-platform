...
import { PLAYER_CATEGORIES, type PlayerCategory } from '../constants.js';

const BENCH: Record<string, Record<string, number | null>> = {
...
export function registerTestTools(server: McpServer): void {
  ...
  server.registerTool('ak_test_log', {
    ...
    if (!player) return error('Spiller ikke funnet', 'Bruk ak_player_list.');
    ...
    if (nextCat && BENCH[nextCat]) {
      ...
      if (params.driver_cs && b.driver_cs && params.driver_cs >= b.driver_cs) passed++;
      if (params.driver_pei && b.driver_pei && params.driver_pei >= b.driver_pei) passed++;
      if (params.putt_3ft_pct && b.putt_3ft && params.putt_3ft_pct >= b.putt_3ft) passed++;
      if (params.stableford_9 && b.stableford_9 && params.stableford_9 >= b.stableford_9) passed++;
      if (params.scramble_pct && b.scramble && params.scramble_pct >= b.scramble) passed++;
      ...
      if (phys >= 2) passed++;
      if (params.preshot_score && b.preshot && params.preshot_score >= b.preshot) passed++;
    }
    const { data, error: err } = await sb.from('test_results').insert({ ...params, categories_passed: passed }).select().single();
    if (err) return handleSupabaseError(err, 'ak_test_log');
    return success({ ...data as Record<string, unknown>, current_category: cat, next_category: nextCat, promotion_ready: passed >= 5, promotion_requirement: '5 av 7 testkategorier' });
  });
  ...
  server.registerTool('ak_test_compare', {
    ...
    if (!tests?.length) return error(`Ingen testresultater for ${player.name}`, 'Bruk ak_test_log for å registrere testresultater.');
    const latest = tests[0]; const prev = tests[1] ?? null;
    const cat = player.category as PlayerCategory; const idx = PLAYER_CATEGORIES.indexOf(cat);
    const nextCat = idx > 0 ? PLAYER_CATEGORIES[idx - 1] : null;
    const curB = BENCH[cat] ?? {}; const nxtB = nextCat ? BENCH[nextCat] ?? {} : {};
    const fields: Array<{ key: string; label: string; bench: string; higher: boolean }> = [
    ...
    const comparison = fields.map(f => {
      const val = latest[f.key as keyof typeof latest] as number | null;
      const pVal = prev?.[f.key as keyof typeof prev] as number | null;
      const nb = nxtB[f.bench] as number | null;
  ...
  server.registerTool('ak_test_history', {
    ...
    if (err) return handleSupabaseError(err, 'ak_test_history');
