...
import { PYRAMID_LEVELS, TRAINING_AREAS, DRILL_DIFFICULTIES } from '../constants.js';

export function registerDrillAgentTools(server: McpServer): void {
...
Examples:
  ...
  - "kort putt under press" → maps to PUTT0-3, PUTT3-5 areas + PR4 level`,
    ...
    if (!player) return error('Spiller ikke funnet', 'Bruk ak_player_list for gyldige IDer.');
    ...
    const metrics: string[] = []; const areas: string[] = [];
    for (const [k, v] of Object.entries(metricMap)) { if (w.includes(k)) metrics.push(...v); }
    for (const [k, v] of Object.entries(areaMap)) { if (w.includes(k)) areas.push(...v); }

    let q = sb.from('drills').select('*').eq('is_approved', true).eq('is_active', true);
    if (metrics.length > 0) q = q.overlaps('trackman_metrics', [...new Set(metrics)]);
    else q = q.or(`name.ilike.%${params.weakness}%,description.ilike.%${params.weakness}%,goal.ilike.%${params.weakness}%`);
    const { data: drills } = await q.limit(params.max_results * 3);

    let areaDrills: typeof drills = [];
    if (areas.length > 0) {
    ...
    const seen = new Set<string>(); const all = [...(drills ?? []), ...(areaDrills ?? [])];
    ...
    const scored = unique.map(d => {
      ...
      for (const word of w.split(/\s+/)) { if (word.length > 2 && txt.includes(word)) score += 2; }
      if (d.trackman_metrics) for (const m of metrics) { if (d.trackman_metrics.includes(m)) score += 3; }
      if (d.training_areas) for (const a of areas) { if (d.training_areas.includes(a)) score += 2; }
  ...
  server.registerTool('ak_drill_import_batch', {
    ...
    if (err) return handleSupabaseError(err, 'ak_drill_import_batch');
    const imported = data ?? [];
  ...
  server.registerTool('ak_drill_coverage_gaps', {
    ...
    if (err) return handleSupabaseError(err, 'ak_drill_coverage_gaps');
    const counts: Record<string, number> = {};
    for (const d of (data ?? [])) { for (const area of (d.training_areas ?? [])) { const k = `${d.pyramid_level}|${area}|${d.difficulty}`; counts[k] = (counts[k] ?? 0) + 1; } }
    const pri: Record<string, number> = { TEK: 5, SLAG: 4, SPILL: 3, FYS: 2, TURN: 1 };
    const gaps: Array<{ pyramid: string; area: string; difficulty: string; current: number; priority: number }> = [];
    for (const pyr of ['TEK','SLAG','SPILL'] as const) {
      for (const area of TRAINING_AREAS) {
        for (const diff of DRILL_DIFFICULTIES) {
          const c = counts[`${pyr}|${area}|${diff}`] ?? 0;
          if (c < params.min_drills_per_combo) gaps.push({ pyramid: pyr, area, difficulty: diff, current: c, priority: (pri[pyr] ?? 1) * (params.min_drills_per_combo - c) });
        }
      }
    }
