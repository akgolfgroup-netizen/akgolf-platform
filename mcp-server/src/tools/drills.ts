...
import { success, error, handleSupabaseError, formatDrillMarkdown, formatPaginationMarkdown } from '../services/responses.js';
import { DrillCreateInput, DrillSearchInput, DrillApproveInput, DrillOutput, PaginatedOutput } from '../schemas/index.js';

export function registerDrillTools(server: McpServer): void {
...
Error: Returns actionable message if required fields missing or enums invalid.`,
    ...
    if (err) return handleSupabaseError(err, 'ak_drill_create');
...
Returns: { total, count, offset, has_more, drills[] }`,
    ...
    if (params.approved_only) q = q.eq('is_approved', true).eq('is_active', true);
    if (params.pyramid_level) q = q.eq('pyramid_level', params.pyramid_level);
    if (params.training_area) q = q.contains('training_areas', [params.training_area]);
    if (params.l_phase) q = q.contains('l_phases', [params.l_phase]);
    if (params.environment) q = q.contains('environments', [params.environment]);
    if (params.difficulty) q = q.eq('difficulty', params.difficulty);
    if (params.trackman_metric) q = q.contains('trackman_metrics', [params.trackman_metric]);
    if (params.sg_area) q = q.eq('sg_area', params.sg_area);
    if (params.tag) q = q.contains('tags', [params.tag]);
    if (params.query) q = q.or(`name.ilike.%${params.query}%,description.ilike.%${params.query}%,goal.ilike.%${params.query}%`);
    if (params.category) {
    ...
    if (err) return handleSupabaseError(err, 'ak_drill_search');
    const drills = data ?? [];
    const total = count ?? 0;
  ...
  server.registerTool('ak_drill_approve', {
    ...
    if (err) return handleSupabaseError(err, 'ak_drill_approve');
    return success(data as Record<string, unknown>, `Drill "${data.name}" ${data.is_approved ? '✓ godkjent' : '✗ avvist'}`);
  });
  ...
  server.registerTool('ak_drill_stats', {
    ...
    if (err) return handleSupabaseError(err, 'ak_drill_stats');
    const drills = data ?? [];
    const byPyramid: Record<string, number> = {}; const byDifficulty: Record<string, number> = {}; const bySG: Record<string, number> = {};
    for (const d of drills) {
      byPyramid[d.pyramid_level] = (byPyramid[d.pyramid_level] ?? 0) + 1;
      byDifficulty[d.difficulty] = (byDifficulty[d.difficulty] ?? 0) + 1;
      if (d.sg_area) bySG[d.sg_area] = (bySG[d.sg_area] ?? 0) + 1;
    }
